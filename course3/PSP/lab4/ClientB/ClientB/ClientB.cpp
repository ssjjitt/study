...
bool GetServer(
    ...
    all.sin_addr.s_addr = INADDR_BROADCAST; // Используем широковещательный адрес
    ...
    if (strcmp(ibuf, call) == 0) {
        ...
        return true; // Успешное получение сообщения
    }
    else {
        memset(from, 0, sizeof(*from)); // Обнуляем указатель на адрес
        return false; // Сообщение не совпадает
    }
