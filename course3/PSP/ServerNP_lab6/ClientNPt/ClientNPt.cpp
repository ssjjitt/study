...
void main()
{
    ...
    try
    {
        if ((hPipe = CreateFile(TEXT("\\\\.\\pipe\\Tube"),
        ...
        wstring sword = L"Hello from client"s;
        wstring word;
        ...
        {
            word = sword + L" " + to_wstring(i);
            DWORD state = PIPE_READMODE_MESSAGE;

            SetNamedPipeHandleState(hPipe, // дескриптор канала
                &state, // указатель на новое состояние канала
                NULL, // указатель на максимальное количество байтов
                NULL); // указатель на интервал задержки

            TransactNamedPipe(hPipe, //дескриптор  канала
                (LPVOID)word.c_str(), // указатель на буфер для записи
                BUFFSIZE, //размер буфера для записи
                answer, // указатель на буфер для чтения
                BUFFSIZE, // размер буфера для чтения
                &byteSize, // количество прочитанных байт
                NULL);  // для асинхронного доступа
