...
int main()
{
    ...
    try
    {
        if ((hPipe = CreateFile(TEXT("\\\\.\\pipe\\Tube"), // символическое имя канала 
            GENERIC_READ | GENERIC_WRITE, // чтение или запись в канал 
            FILE_SHARE_READ | FILE_SHARE_WRITE, // режим совместного использования
            NULL, OPEN_EXISTING, NULL, //  атрибуты безопасности, флаг открытия канала , флаги и атрибуты
            NULL)) == INVALID_HANDLE_VALUE) // дополнительные атрибуты
        {
        ...
        for (int i = 0; i < 1000; i++)
        {
            ...
            WriteFile(hPipe, word.c_str(), BUFFSIZE, &byteSize, NULL);

            sucsess = ReadFile(hPipe, answer, BUFFSIZE, &byte, NULL);

            _tprintf_s(TEXT("%s\n"), answer);
        }

        WriteFile(hPipe, "", 1, &byteSize, NULL);
        CloseHandle(hPipe);
    }
