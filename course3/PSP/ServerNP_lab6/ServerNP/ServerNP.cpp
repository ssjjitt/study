...
int main()
{
    ...
    try
    {
        if ((hPipe = CreateNamedPipe(TEXT("\\\\.\\pipe\\Tube"), // [in] символическое имя канала 
            ...
            NULL, //[in] размер выходного буфера
            NULL,// [in] размер входного буфера
            INFINITE, //[in] время ожидания связи с клиентом
            NULL)) == INVALID_HANDLE_VALUE) // [in] атрибуты безопасности
            throw SetPipeError("create:", GetLastError());

        cout << "wait connection...\n";

        size_t count = 0;

        do {
            if (!ConnectNamedPipe(hPipe, NULL)) // дескриптор именованного канала и параметр который используется для асинхр. связи
            ...
            do {
                 sucsess = ReadFile(hPipe, msg, BUFFSIZE, &byteSize, NULL);
