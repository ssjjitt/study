...
int main()
{
    ...
    try
    {
        if ((hPipe = CreateFile(TEXT("\\\\127.0.0.1\\pipe\\Tube"), // символическое имя канала 
            GENERIC_READ | GENERIC_WRITE, // чтение или запись в канал 
            0, // режим совместного использования
            NULL, 
            OPEN_EXISTING, 
            0, //  атрибуты безопасности, флаг открытия канала , флаги и атрибуты
            &sa)) == INVALID_HANDLE_VALUE) // дополнительные атрибуты
        {
        ...
        for (int i = 0; i < 1000; i++) {
            snprintf(msg, sizeof(msg), "Сообщение %d", i);
