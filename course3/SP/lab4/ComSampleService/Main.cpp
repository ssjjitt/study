...
HRESULT ExeUnregisterServer()
{
    ...
    {
        hr = hrTemp;
        printf("Failed to unregister service, hr = %08lX\n", hr);
    }
    ...
    {
        hr = hrTemp;
        printf("Failed to unregister AppID, hr = %08lX\n", hr);
    }
    ...
    for (int i = 0; i < ARRAYSIZE(g_Classes); ++i)
    {
        ...
        {
            hr = hrTemp;
            printf("Failed to unregister class %ls, hr = %08lX\n", g_Classes[i].pszName, hr);
        }
    }
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            for (int i = 0; (i < ARRAYSIZE(g_Classes)) && SUCCEEDED(hr); ++i)
            {
                ...
                if (FAILED(hr))
                {
                    printf("Failed to register class %ls, hr = %08lX\n", g_Classes[i].pszName, hr);
                }
            }
        }
        else
        {
            printf("Failed to register AppID, hr = %08lX\n", hr);
        }
    }
    else
    {
        printf("Failed to register service, hr = %08lX\n", hr);
    }
...
int __cdecl wmain(int argc, __in wchar_t * argv [])
{
    ...
    {        
        ...
        if (pszToken != nullptr)
        {
            if (_wcsicmp(pszToken, L"RegisterServer") == 0)
            {
                ...
                printf("RegisterServer done, hr = %08lX\n", hr);
            }
            else if (_wcsicmp(pszToken, L"UnregisterServer") == 0)
            {
                hr = ExeUnregisterServer();
                printf("UnregisterServer done, hr = %08lX\n", hr);
            }
    ...
    if (fStartService)
    {
        ...
        rgServiceTable[0].lpServiceName = (LPWSTR)SERVICE_NAME;
