...
# Линейная регрессия:
# Это просто попытка найти прямую, которая максимально хорошо описывает данные.
# На графике видно, как она находит лучшее приближение, но она не всегда идеальна, особенно если данные немного шумные.
# Линейная регрессия хороша для простых задач.

mglearn.plots.plot_linear_regression_wave() # вычисляем параметры w и b
plt.show()
...
# Гребневая регрессия:
# Здесь добавляется кое-какая «штрафная» сила, чтобы не перегрузить модель.
# Она пытается не слишком сильно «поглощать» данные и штрафует за большие веса.
# Чем выше параметр alpha, тем сильнее эта регуляризация, и тем меньше значения коэффициентов.
# Гребневая регрессия помогает, если признаков много, и они могут «запутывать» модель.
...
# Изменение параметра alpha для гребневой регрессии
...
plt.hlines(0, 0, len(lr.coef_))  # Горизонтальная линия на 0 для сравнения
plt.ylim(-25, 25)
plt.legend()
plt.show()
...
# Лассо:
# Это как гребневая регрессия, но она еще и «обрезает» неважные признаки, заставляя их стать равными нулю.
...
# Изменение параметра alpha для Lasso
...
# Визуализация коэффициентов для разных значений alpha в Lasso
