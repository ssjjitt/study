...
# SVM (Support Vector Machine): Это алгоритм, который ищет максимальный отступ (margin)
# между двумя классами, то есть точки, которые лежат как раз на границе между классами.

# Логистическая регрессия определяет, насколько вероятно, что объект относится к одному из классов,
# и на основе этого принимает решение.
...
# Чем меньше C, тем сильнее регуляризация → модель более "плавная",
# но может плохо описывать данные (недообучение).
mglearn.plots.plot_linear_svc_regularization()
plt.show()

print("---------------------------")

# C - компромиссный параметр. чем больше - тем меньше регуляризация
...
# значения весов желтеньким больше - модель выжимает максимум
...
# в параметре penalty
# L1 регуляризация "жмёт" коэффициенты сильнее.
# Она может занулять некоторые веса → автоматический отбор признаков.
# При малом C почти все веса 0 → модель игнорит большинство признаков.
