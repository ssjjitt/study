// Дан указатель p1 на корень бинарного дерева. Написать функцию вывода количества листьев дерева, которые являются правыми дочерними вершинами.
...
void main()
{
	...
	for (;;)
	{
		...
		cout << "8 - количество правых дочерних вершин\n";
		cout << "9 - выход\n";
		cout << "ваш выбор?\n";
		cin >> choice;
		cout << "\n";
...
int task(Tree* tree)
{
	...
	{
		task(tree->Right);
		sum++;				// подсчет количества листьев, которые являются правыми дочерними вершинами
	}
	if (tree->Left != NULL)
		task(tree->Left);
