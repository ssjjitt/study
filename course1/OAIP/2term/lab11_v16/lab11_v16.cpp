...
int sum = 1, result3 = 0, counter2 = 0, counter = 0;
...
int task(Tree* tree, int level);		// Подсчёт листьев
...
void main()
{
	...
	for (;;)
	{
		...
		case 8:	 cout << "Количество дочерних вершин = " << task(Root, 0) << endl; break;
		case 9:  exit(0);
		}
	}
}
...
int task(Tree* tr, int level)
{
	if (tr)
	{
		task(tr->Right, level + 1);	//вывод правого поддерева	
		if (tr->key % 2 == 0)
			counter++;

		task(tr->Left, level + 1);	//вывод левого поддерева
	}
	return counter;
}